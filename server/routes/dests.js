  ...
  DestAccom.findAll({
    ...
    .then((destAccoms) => {
      if (!destAccoms || destAccoms.length === 0) {
        res.json({
          error: "No accommodations found for the specified destination",
        });
      } else {
  ...
  DestTrans.findAll({
    ...
    .then((destTrans) => {
      ...
      if (!destTrans || destTrans.length === 0) {
        res.json({
          error: "No transportation found for the specified destination",
        });
      } else {
  ...
  DestRestaurant.findAll({
    ...
    .then((destRests) => {
      if (!destRests || destRests.length === 0) {
        res.json({
          error: "No restaurants found for the specified destination",
        });
      } else {
        // console.log('destAccoms:', destAccoms);
