...
import RcmActCardCustom from "./rcm-act-card-custom"
...
export default class ActCustom extends React.Component {
    constructor(props) {
        ...
        this.loadAct = this.loadAct.bind(this)
    ...
    save() {
        ...
        localStorage.setItem(`act_dest${destId}`, jsonFile)
                        ...
                        {this.state.filteredData
                            ...
                            .map((act) => (
                                <div className="custom-card col col-6 col-sm-6 col-md-3" onClick={() => {
                                    if (!this.state.activeList.includes(act.id)) {
                                        this.setState({ activeList: this.state.activeList.concat(act.id) }, () => {
                                            console.log("Active List: ", this.state.activeList)
                                        })
                                    } else {
                                        const index = this.state.activeList.indexOf(act.id);
                                        if (index !== -1) {
                                    ...
                                    <RcmActCardCustom act={act} active={this.state.activeList.includes(act.id)}></RcmActCardCustom>
                                </div>
                            ))}
                    </div>
